...
func (mt *MemTable) Put(key, value []byte) error {
	...
	if idx < len(mt.entries) && bytes.Equal(mt.entries[idx].Key, keyCopy) {
		...
		mt.entries = append(mt.entries, KeyValue{})
		copy(mt.entries[idx+1:], mt.entries[idx:])
		mt.entries[idx] = KeyValue{Key: keyCopy, Value: valueCopy}
	}
...
func (mt *MemTable) GetRange(startKey, endKey []byte) []KeyValue {
	mt.mu.RLock()
	defer mt.mu.RUnlock()
	...
	// This handles cases where findIndex returns a position but the key there is actually less
	// (can happen with certain key orderings)
	if bytes.Compare(mt.entries[startIdx].Key, startKey) < 0 {
		// All keys are less than startKey, return empty
		return []KeyValue{}
	}
	...
	if endKey == nil {
		...
		// If endKey exists exactly, we want to include it
	...
	// Extract the range
	result := make([]KeyValue, 0, endIdx-startIdx)
	for i := startIdx; i < endIdx && i < len(mt.entries); i++ {
		key := mt.entries[i].Key

		// Double-check that key is >= startKey (should always be true at this point)
		if bytes.Compare(key, startKey) < 0 {
			continue
		}

		// Stop if we've reached or passed endKey
		...
		copy(kv.Key, key)
		copy(kv.Value, mt.entries[i].Value)
...
func (mt *MemTable) GetAllEntries() []KeyValue {
	mt.mu.RLock()
	defer mt.mu.RUnlock()

	// Return a deep copy
	result := make([]KeyValue, len(mt.entries))
